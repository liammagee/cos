...
<div>
    <a href="view_issue_matrix.jsp#" id="simpleRanking">Simple Ranking</a> | <a href="view_issue_matrix.jsp#" id="criteriaRanking">Criteria-based Ranking</a>
</div>
...
<form id="updateRanking" action="/rankIssues" method="post">
    ...
    <input name="action" value="updateIssueMatrix" type="hidden"/>
    <input name="criterionID" value="<%= request.getParameter("criterionID") %>" type="hidden"/>
<table>
    <%
        ...
        IssueMatrix matrix = ahp.getIssueMatrix(request.getParameter("criterionID"));
        List<MatrixRow> matrixRows = matrix.getMatrixRows();
    %>
    <tr>
        <th colspan="<%= matrixRows.size() + 1 %>">
            <%= matrix.getCriterion().getName() %>
        </th>
    </tr>
    <tr>
        <th colspan="<%= matrixRows.size() + 1 %>">
            Consistency statistics
            <%--Consistency statistics (will only update after 'Update Issue Matrix' is clicked).--%>
        </th>
    </tr>
    <tr>
    ...
    <tr>
        ...
        <td colspan="<%= matrixRows.size() %>"><span style="border: 1px solid #00; background-color: #ff0000; font-weight: bold; color: #fff" ><%= Boolean.toString(matrix.isConsistent()).toUpperCase() %></span></td>
    </tr>
    ...
    %>
        <tr>
            <td>
                <strong><%= ahp.getIssues().get(rowCounter).getName() %></strong>
            </td>
            <%
                int columnCounter = 0;
    ...
    <p><input type="submit" value="Update Issue Matrix" /> </p>
</form>
